...
class linkedin_soup(object):
    ...
    def get_post_date(self):
        ...
        if _unit in ['minutes', 'minute', 'hours', 'hour']:
            self.post_date = datetime.now().strftime("%d/%m/%Y")
        else:
            try:
                ...
                self.post_date = np.nan
                return 
            self.post_date = _post_date.strftime("%d/%m/%Y")
...
def scrape_page(driver, cursor, pbar = None):
    ...
    for elem in job_elems:
        ...
        dup_replace_list = ', '.join([col + '=' + f"VALUES({col})" for col in job_key_list])
        job_insert_sql = f"INSERT INTO job ({job_key_list}) VALUES ({job_value_list}) ON DUPLICATED KEY UPDATE {dup_replace_list};"
...
def scrape_pages(driver, cursor, page_num = 40):
    skill_dfs = []
    job_dfs = []
    ...
    with tqdm(total=page_num * 25, position=1, leave=True) as pbar:
        with tqdm(range(1, page_num + 1), position=0, leave=True, ncols = 1, 
            ...
            for page in page_bar:
                ...
                _skill_df, _job_df = scrape_page(driver, pbar, cursor)
